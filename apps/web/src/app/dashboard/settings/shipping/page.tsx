...
export default function ShippingPage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl lg:text-3xl font-bold text-white">Perfis de Frete</h1>
                <p className="text-white/40 mt-1">Configure as regras de custo de frete para o cálculo de lucro</p>
            </div>

            <div className="card">
                <h2 className="text-lg font-semibold text-white mb-4">{editId ? 'Editar Perfil' : 'Novo Perfil'}</h2>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <input className="input-field" placeholder="Nome do Perfil" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
                    <select className="input-field" value={form.rule_type} onChange={(e) => setForm({ ...form, rule_type: e.target.value })}>
                        ...
                        <option value="fallback_default">Padrão de Backup</option>
                    </select>
                    <input className="input-field" placeholder="Custo Médio de Frete" type="number" step="0.01" value={form.avg_shipping_cost} onChange={(e) => setForm({ ...form, avg_shipping_cost: e.target.value })} />
            ...
            <div className="card">
                {loading ? <p className="text-white/30 text-sm">Carregando...</p> : profiles.length === 0 ? <p className="text-white/30 text-sm">Sem perfis de frete.</p> : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead><tr className="border-b border-white/10">
                                <th className="text-left py-3 px-2 text-white/40">Nome</th>
                                <th className="text-left py-3 px-2 text-white/40">Regra</th>
                                <th className="text-right py-3 px-2 text-white/40">Custo Médio</th>
